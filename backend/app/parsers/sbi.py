...
class SBIParser(BaseParser):
    ...
    def can_parse(self, content: str | bytes) -> bool:
        ...
        return "STATE BANK" in text.upper() or "SBI" in text.upper()
    ...
    def _parse_pdf(self, file_path: str) -> list[RawTransaction]:
        ...
        row_pattern = re.compile(
            r"(\d{2}\s+\w{3}\s+\d{4})\s+(.+?)\s+([\d,]+\.\d{2})\s+(Dr|Cr)",
            re.IGNORECASE,
        )
        ...
        logger.info(f"SBI PDF parsed: {len(transactions)} transactions")
        return transactions
    ...
    def _parse_tabular(self, file_path: str, ext: str) -> list[RawTransaction]:
        ...
        try:
            ...
            logger.error(f"SBI tabular parse error: {e}")
        return transactions
